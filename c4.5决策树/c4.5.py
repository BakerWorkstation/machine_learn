...
def createDateset(filename):
    with open(filename, 'r')as csvfile:
        ...
        labelType = [
                     'continuous', 'uncontinuous', 'uncontinuous', 'uncontinuous', 'continuous',
                     'continuous', 'uncontinuous', 'continuous', 'continuous', 'continuous',
                     'continuous', 'uncontinuous', 'continuous', 'uncontinuous', 'uncontinuous',
                     'continuous', 'continuous', 'continuous', 'continuous', 'continuous',
                     'uncontinuous', 'uncontinuous', 'continuous', 'continuous', 'continuous',
                     'continuous', 'continuous', 'continuous', 'continuous', 'continuous',
                     'continuous', 'continuous', 'continuous', 'continuous', 'continuous',
                     'continuous', 'continuous', 'continuous', 'continuous', 'continuous',
                     'continuous'
...
def splitContinuousDataSet(dataSet,feature,continuousValue):#根据连续值来划分数据集
    ...
    for vec in dataSet:
        ...
        if float(vec[feature])>continuousValue:#如果feature列的值比最佳分割点的大，则放在biggerDataSet中，否则放在smallerDataSet中
...
def main():
    dataSetName = r"kddcup.data_10_percent"
    print('加载训练数据')
    ...
    # print(Tree)
    # storeTree(Tree, r'tree1.txt')  # 保存决策树，避免下次再生成决策树

    print('读取树')
    Tree=grabTree(r'tree1.txt')#读取决策树，如果已经存在tree.txt可以直接使用决策树不需要再次生成决策树
    print('预测')
    total, correct, error  = test(Tree, label, r'corrected',labelType,mydate)
